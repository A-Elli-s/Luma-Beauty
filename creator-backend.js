(() => {
  ...
  const cleanCode = value => String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]/g, '').slice(0, 40);
  ...
  function captureReferral() {
    ...
    if (API_BASE) post({ action:'click', creatorCode:incoming, pagePath:window.location.pathname }).catch(() => {});
  }
  ...
  function currentReferral() {
    ...
    if (!code || !timestamp || Date.now() - timestamp > ATTRIBUTION_DAYS * 86400000) {
      localStorage.removeItem(REF_KEY); localStorage.removeItem(REF_TS_KEY); return '';
    }
    return code;
  }
  ...
  async function submitApplication(form, status) {
    ...
    if (!API_BASE) throw new Error('Application service is temporarily unavailable.');
    status.textContent = 'Submitting application…';
    const result = await post(payload);
    localStorage.removeItem('lumaCreatorApplicationDraft');
    form.reset();
    status.textContent = `Application received. Reference: ${result.applicationId}.`;
    status.classList.add('saved');
  }

  document.addEventListener('DOMContentLoaded', () => {
    captureReferral();
    const form=document.getElementById('creatorApplication'); const status=document.getElementById('creatorFormStatus');
    if(!form||!status)return;
    form.addEventListener('submit', async event => {
      event.preventDefault(); if(!form.reportValidity())return;
      const button=form.querySelector('button[type="submit"]'); if(button)button.disabled=true;
      try{await submitApplication(form,status);}catch(error){status.textContent=error.message;status.classList.remove('saved');}
